...
const TransferForm = () => {
  ...
  const [oppositeOption, setOption] = useState("SOS");
  ...
  const calculate = (selected, opposite, entered) => {
    fetch(
      `https://faremitt.up.railway.app/api/paircode/${selected}/${opposite}/${entered}`
    )
      .then((res) => res.json())
      .then((res) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (token) {
      ...
      openModal(); // Assuming openModal is a function to open the modal
    }
  };
  ...
  return (
    ...
    >
      {" "}
      <div className="flex overflow-hidden z-50 justify-center items-start w-full md:w-[461px] h-full relative gap-2 rounded-xl bg-white border border-[#f7f6f8]">
        <div className="flex flex-col justify-start items-start flex-grow-0 flex-shrink-0 w-full absolute left-0 p-1.5 top-3 md:left-4 gap-3">
          <Selection
            ...
            title="you send"
          />
          <div className="flex justify-between items-center flex-grow-0 flex-shrink-0 w-full  px-5 py-3 rounded-xl  border border-[#c7c4d4]">
          ...
          <div className="flex flex-col justify-start items-start flex-grow-0 flex-shrink-0 w-full n gap-5 px-5 py-3 rounded-xl">
            <div className="flex flex-col justify-start items-start self-stretch flex-grow-0 flex-shrink-0 gap-4 pl-4 border-t-0 border-r-0 border-b-0 border-l border-[#dcdae1]">
